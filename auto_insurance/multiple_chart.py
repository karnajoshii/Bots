...
# Identify axes
def identify_axes(data, user_query):
    """Identify the most appropriate X, Y, and group-by axes for visualization."""
    ...
    # Detect group-by patterns in query (e.g., "gender-wise")
    group_by_match = re.search(r'\b(?:by|per|across|for each|grouped by|break(?:ing)? down by|-wise)\s+([a-zA-Z\s]+)\b', user_query.lower())
    group_by_cols = []
    if group_by_match:
        group_terms = group_by_match.group(1).strip().split('and')
        for term in group_terms:
            term = term.strip()
            if term in ['gender', 'state', 'vehicle class', 'coverage', 'marital status']:
                col_name = term.title().replace(' ', '_')
                if col_name in keys:
    ...
    if categorical_keys and numeric_keys:
        preferred_x = ["State", "Income", "Gender", "Marital_Status", "Vehicle_Class", "Coverage", "Year"]
        for preferred in preferred_x:
            if preferred in categorical_keys:
                x_axis = preferred
                break
        if not x_axis:
            x_axis = categorical_keys[0]
        y_axis = numeric_keys[0]
        
        # Assign group_by if a second categorical key exists
        remaining_cats = [k for k in categorical_keys if k != x_axis]
        if remaining_cats and len(group_by_cols) > 1:
            group_by = next((col for col in remaining_cats if col in group_by_cols), None)
        elif remaining_cats:
            group_by = remaining_cats[0]
    
    elif categorical_keys:
        count_columns = [k for k in keys if 'count' in k.lower() or 'total' in k.lower() or 'avg' in k.lower()]
        if count_columns:
            y_axis = count_columns[0]
        else:
            y_axis = "COUNT(*)"
        x_axis = categorical_keys[0]
        ...
        if "Income" in numeric_keys:
            x_axis = "Income"
            y_axis = [k for k in numeric_keys if k != "Income"][0]
        else:
            x_axis = numeric_keys[0]
            y_axis = numeric_keys[1]
...
def should_visualize(user_query, result_data):
    ...
    if explicit_request and not suitable:
        ...
        x_axis, y_axis, group_by = identify_axes(result_data, user_query)
...
def prepare_visualization_data(result_data, user_query):
    """Format result data for chart visualization with proper axes and grouped data."""
    if isinstance(result_data, dict) and "data" in result_data:
    ...
    x_axis, y_axis, group_by = identify_axes(chart_data, user_query)
    ...
    # NEW: Handle grouped data for multiple bars
    formatted_data = []
    if group_by:
        # Group data by x_axis and group_by
        grouped = {}
        for item in chart_data:
            ...
            if x_value is None or y_value is None or group_value is None:
                continue
                
            try:
                y_float = float(y_value) if isinstance(y_value, (int, float)) else float(y_value.replace(',', '')) if isinstance(y_value, str) and y_value.replace(',', '').replace('.', '').isdigit() else 0
            except (ValueError, AttributeError):
                y_float = 0
                
            if x_value not in grouped:
                grouped[x_value] = []
            grouped[x_value].append({
    ...
    return formatted_data, x_axis, y_axis, group_by

# Determine chart type
def determine_chart_type(user_query, chart_data, x_axis, y_axis, group_by):
    ...
    is_percentage = "percentage" in y_axis.lower() or all(float(item["y"]) <= 100 for item in chart_data if "y" in item)
    
    # NEW: Prefer grouped bar chart if group_by is present
    if group_by:
        return "bar"
...
def generate_user_friendly_response(user_query, result_data, sql_query, has_visualization=False, group_by=None):
...
5. For grouped visualizations (when Grouped by is not None):
   - Mention the grouping in the response (e.g., "I've created a bar chart showing claim counts by state, with separate bars for each gender.")
   - Highlight key insights from the grouped data.
6. For other queries, highlight key insights from the data.
7. Use natural language, avoiding technical terms.
8. Keep answers concise but complete (2-4 sentences for non-list queries).
9. If there's a visualization, mention it briefly.
10. Be precise with numerical results.
...
User question: "gender-wise representation for number of claims for each state"
Response: "I've got the claim counts for each state, broken down by gender. For example, in California, there are 300 claims from males and 400 from females. I've created a bar chart showing claim counts by state, with separate bars for each gender."
    ...
    try:
        ...
        with get_openai_callback() as cb:
            ...
            if sql_query.startswith("Answer from history:"):
                ...
                visualization_data = None
                group_by = None  # Initialize group_by to None to prevent undefined variable error
                
                if should_viz:
                    chart_data, x_axis, y_axis, group_by = prepare_visualization_data(result_data, user_query)
                    
                    if chart_data and x_axis and y_axis:
                        chart_type = determine_chart_type(user_query, chart_data, x_axis, y_axis, group_by)
                        
                        if chart_type == "pie" and len(chart_data) > 7:
                            chart_data = sorted(chart_data, key=lambda x: float(x["y"]) if "y" in x else sum(g["y"] for g in x["groups"]), reverse=True)[:7]
                        ...
                        visualization_data = {
                            ...
                            "group_label": group_by.replace("_", " ").title() if group_by else None
                        }
                        current_query_context.x_axis = x_axis
                        current_query_context.y_axis = y_axis
                
                response_text = generate_user_friendly_response(user_query, result_data, sql_query, should_viz, group_by)
...
- If the user says "Good morning",
 ...
 respond with an enhanced morning greeting
...
def handle_visualization_only(user_query, chat_id):
    ...
    if chart_type == "bar" and group_by:
        response_text = f"Here's a grouped bar chart showing {y_axis.replace('_', ' ').title()} across different {x_axis.replace('_', ' ').title()} values, with separate bars for each {group_by.replace('_', ' ').title()}."
    elif chart_type == "bar":
